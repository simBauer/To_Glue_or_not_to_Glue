...
# - Extended with absolute pose for TUM dataset
# -----------------------------------------------------------
...
class ImagePairs(BaseDataset, torch.utils.data.Dataset):
    default_conf = {
        ...
        "extra_data": None,  # relative_pose, homography, absolute_pose
    }
    ...
    def __getitem__(self, idx):
        ...
        if self.conf.extra_data == "relative_pose":
            ...
            # Facade image has not camera matrix, but is also scaled
            data["facade"] = {}
            data["facade"]["scales"] = data0["scales"]
            data["facade"]["ori_size"] = data0["original_image_size"]
            
            data["view1"]["camera"] = parse_camera(pair_data[2:11]).scale(
                data1["scales"]
            )
            # Absolute pose can be read with same function
            data["T_0to1"] = parse_relative_pose(pair_data[11:23])

            # Load information for converting pixels in facade image to world coordinates
            with open("data/tum_facade/texture_coords.json", "r", encoding='utf-8') as f:
                input = json.load(f)
            st_edges = np.fromstring(input[name0.rsplit("/", 1)[-1]]["st"], sep=" ")
            xyz_edges = np.fromstring(input[name0.rsplit("/", 1)[-1]]["xyz"], sep=" ")

            data["facade"]["st_edges"] = torch.tensor(st_edges, dtype=torch.float32).view(-1, 2)
            data["facade"]["xyz_edges"] = torch.tensor(xyz_edges, dtype=torch.float32).view(-1, 3)
