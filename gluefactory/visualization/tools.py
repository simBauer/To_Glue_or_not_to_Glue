...
# - Added WorldMatchesPlot based on EpipolarMatchesPlot for TUM dataset evaluation
# -----------------------------------------------------------
...
from ..eval.tum import get_world_coordinates_from_pixels
from ..geometry.poses import projection_error
...
class WorldMatchesPlot:
    plot_name = "proj_error"
    required_keys = ["keypoints0", "keypoints1", "matches0", "T_0to1", "view1", "facade"]
    ...
    def __init__(self, fig, axes, data, preds):
        ...
        for i, name in enumerate(preds):
            ...
            kpm0_w = kpts0_world[valid, :]

            # errors = projection_error(kpm0_w, kpm1, camera1, T_0to1)

            # TUM-evaluation specific:
            # Apply the scaling of the images to width 1600 for consistency with the results
            sc = 1600./data["view1"]["original_image_size"][0, 0]
            kpm1_scaled = kpm1 * sc
            camera1_scaled = camera1.scale(sc)
            errors = projection_error(kpm0_w, kpm1_scaled, camera1_scaled, T_0to1)
