...
class ImagePreprocessor:
    ...
    def __call__(self, img: torch.Tensor, interpolation: Optional[str] = None) -> dict:
        ...
        if self.conf.create_mask:
            # Mask all pixels which are absolute black or white
            mask = torch.all((img == 0) | (img == 1.), dim=-3)
            if not mask.any():
                mask = None
        else:
            mask = None
        ...
        if self.conf.resize is not None:
            ...
            # Resize the mask without interpolation
            if mask is not None:
                mask = kornia.geometry.transform.resize(
                    mask[None].float(),
                    size,
                    side=self.conf.side,
                    antialias=False,
                ...
                # Convert to numpy uint8 (for openCV compatibility)
                mask = mask.cpu().numpy().astype(np.uint8)*255.
                # Erode and dilate the mask to avoid interpolation artifacts
                mask = cv2.erode(mask, np.ones((3, 3), np.uint8), iterations=1)
                dil = np.ones((5, 5), np.uint8)
                mask = cv2.dilate(mask, dil, iterations=5)
                mask = 255 - mask
